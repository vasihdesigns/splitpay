  ...
  return (
    <SafeAreaView className="flex-1 bg-primary-950">
      ...
      <ScrollView
        ...
        refreshControl={
          <RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); fetchActivity(); }} tintColor="#6366f1" />
        }
      >
        {loading ? (
          ...
          <View className="items-center mt-20">
            ...
            <Text className="text-white font-bold text-lg">No activity yet</Text>
          ...
          activities.map((item) => (
            <View
              key={item.id}
              className="bg-primary-900 rounded-xl p-4 mb-2 flex-row items-center gap-3"
            >
              ...
              <View className="flex-1">
                <Text className="text-white font-medium">
                  {item.actor_name}{' '}
                  <Text className="text-primary-300 font-normal">
                    {item.type === 'expense_added' ? 'added' : item.type.replace('_', ' ')}
                  </Text>{' '}
                  {item.description}
                </Text>
                {item.amount !== undefined && (
                  <Text className="text-primary-400 text-sm">{formatCurrency(item.amount)}</Text>
                )}
                <Text className="text-primary-500 text-xs mt-0.5">{formatDate(item.created_at)}</Text>
              </View>
            </View>
          ))
        )}
